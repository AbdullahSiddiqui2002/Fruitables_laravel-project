    ...
    <body>
                ...
                <nav class="navbar navbar-light bg-white navbar-expand-xl">
                    ...
                    <div class="collapse navbar-collapse bg-white" id="navbarCollapse">
                        <div class="navbar-nav mx-auto">
                            ...
                            <a href="/manage-product" class="nav-item nav-link active">Manage Product</a>
        ...
        <div class="container-fluid page-header py-5">
            ...
            <ol class="breadcrumb justify-content-center mb-0">
                ...
                <li class="breadcrumb-item"><a href="/manage-product">Manage Product</a></li>
                    ...
                    <div class="row g-4">
                        <div class="col-lg-12">
                            <form action="/updateproduct/{{$product['id']}}" method="post" enctype="multipart/form-data">
                                ...
                                @enderror
                                <textarea class="w-100 form-control border-0 mb-4 mt-4" rows="5" cols="10"
                                    placeholder="Enter Product Description" name="product_description" value="{{ old('product_description')}}">{{$product['description']}}</textarea>
                                @error('product_description')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                                <input type="number" class="w-100 form-control border-0 py-3 mb-4 mt-4"
                                    placeholder="Enter Product Price" name="product_price"
                                    value="{{ old('product_price', $product['price']) }}">
                                @error('product_price')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                                <select class="w-100 form-control border-0 py-3 mb-4 mt-4" name="category_id" id="">
                                    @if ($product['category'] == 3)
                                        <option class="w-100 form-control border-0 py-3 mb-4" value="{{ $product['category'] }}" selected>Fruit</option>
                                    @elseif ($product['category'] == 4)
                                        <option class="w-100 form-control border-0 py-3 mb-4" value="{{ $product['category'] }}" selected>Vegetable</option>
                                    @elseif ($product['category'] == 5)
                                        <option class="w-100 form-control border-0 py-3 mb-4" value="{{ $product['category'] }}" selected>Bread</option>
                                    @else
                                        <option class="w-100 form-control border-0 py-3 mb-4" value="{{ $product['category'] }}" selected>Meat</option>
                                    @endif
                                    @foreach ($categories as $cat)
                                        @if ($cat['id'] != $product['category'])
                                            <option class="w-100 form-control border-0 py-3 mb-4" value="{{ $cat['id'] }}">{{ $cat['category_name'] }}</option>
                                        @endif
                                    @endforeach
                                </select>
                                @error('category_id')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                                <input type="file" class="w-100 form-control border-0 py-3 mb-4 mt-4"
                                    placeholder="Product Image" name="product_image"
                                    value="{{ old('product_image', $product['image']) }}">
                                @error('product_image')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                                <button class="w-100 btn form-control border-secondary py-3 mt-4 bg-white text-primary"
                                    name="updateproduct" type="submit">Update Product</button>
                            </form>
                        </div>
                    </div>
                </div>
